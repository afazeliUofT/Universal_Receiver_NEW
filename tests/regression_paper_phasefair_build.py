...
def main() -> None:
    parser = argparse.ArgumentParser(description="Regression for phase-aware paper configured-reservoir comparator.")
    parser.add_argument("--config", default="configs/smoke_paper_phasefair_comparison.yaml", type=str)
    ...
    classical_receivers, classical_estimators, artifacts = build_classical_baseline_suite(
        cfg=cfg,
        tx=tx,
        channel=channel,
        paths=paths,
    ...
    if BASELINE_PAPER_CFGRES_PHASEAWARE_LS_LMMSE not in classical_estimators:
        raise AssertionError("Phase-aware paper configured-reservoir estimator was not built.")
    if BASELINE_PAPER_CFGRES_PHASEAWARE_LS_LMMSE not in classical_receivers:
        raise AssertionError("Phase-aware paper configured-reservoir receiver was not built.")

    basis_path = artifacts.get("paper_cfgres_phasefair_basis")
    cov_path = artifacts.get("paper_cfgres_phasefair_covariances")
    if basis_path is None or cov_path is None:
        raise AssertionError("Phase-aware paper configured-reservoir artifacts were not registered.")
    if not Path(basis_path).exists():
        raise AssertionError(f"Missing phase-aware paper basis artifact: {basis_path}")
    if not Path(cov_path).exists():
        raise AssertionError(f"Missing phase-aware paper covariance artifact: {cov_path}")
    ...
    if not bool(tf.reduce_all(tf.math.is_finite(tf.math.real(h_hat))).numpy()):
        raise AssertionError("Phase-aware paper h_hat contains non-finite real values.")
    if not bool(tf.reduce_all(tf.math.is_finite(tf.math.imag(h_hat))).numpy()):
        raise AssertionError("Phase-aware paper h_hat contains non-finite imaginary values.")
    if not bool(tf.reduce_all(tf.math.is_finite(tf.cast(err_hat, tf.float32))).numpy()):
        raise AssertionError("Phase-aware paper err_hat contains non-finite values.")

    receiver = classical_receivers[BASELINE_PAPER_CFGRES_PHASEAWARE_LS_LMMSE]
    call_receiver(receiver, y, no)
    print("Phase-aware paper configured-reservoir regression passed.")
